module.exports = function(jstatic) {
    ...
    jstatic.registerGenerator("destination", function(iter, params, flow, depends, data) {
        ...
        if(_.isString(params.dest)) {
            pathFunc = function(entry) {
                return _.template(params.dest, entry, {
                    interpolate: /\$\((.+?)\)/g
                });
            }
        ...
        return function() {
            ...
            var pathFuncParams = _.extend({sep: path.sep, outExt: flow.outExt, dest: flow.dest}, entry);
            entry.destPath = path.join(flow.dest, pathFunc(_.extend(pathFuncParams)));
            return entry;
        };
    });
