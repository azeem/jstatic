module.exports = (function() {
     ...
     * the file content.
     */
    jstatic.Flow = function(src, dest, name, options, gruntConf) {
        this.name = name;
        this.src = src;
        this.dest = dest;
        this.outExt = options.outExt || ".html";
        this.depends = options.depends || [];
        this.stash = options.stash?true:false;
        this.gruntConf = gruntConf;
    ...
    _.extend(jstatic.Flow.prototype, {
        execute: function(options, data, depends) {
            ...
            if(scanCount > 0) {
                grunt.log.oklns("Scanned " + scanCount + " items");
            }

            if(this.stash) {
                this.gruntConf.set("jstatic.stash."+this.name, this.entries);
            }
     ...
     * Jstatic task
     */
    jstatic.Task = function(filesArray, options, gruntConf) {
        this.flows = _.chain(filesArray).map(function(item, index) {
            var name = item.name || "flow_" + index;
            var flow = new jstatic.Flow(item.src, item.dest, name, item, gruntConf);
